...
sub getPhoneMAC
{
	...
	my %fields = ($PBXgetPhoneMAC => '','6603ff00'=>''); #thats weird, depending on the command its the same entry for elin as it is for mac
        ...
        if ($node->last_command_succeeded())
	{
		...
		print "DEBUG ".$hash_ref->{$PBXgetPhoneMAC}." ".$hash_ref->{'6603ff00'}." ";# if $debug;
...
sub getELIN
{
		...
		{
			print ("ext: $ext  current_elin: ".$hash_ref->{$PBXgetELIN}." tms_elin: $elin\n");
		}
		else
		{
...
#	}
	...
	my $match = substr(matchMACToELIN($mac->get_mac(),$voipphone->{$PBXgetExtension}), -5, 5);
